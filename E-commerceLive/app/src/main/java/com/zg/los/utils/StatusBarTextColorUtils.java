...
//     */
//    public static void setStatusBarLightMode(Window window, @ColorRes int color) {
...
//    }
    ...
    public static void setStatusTextColorMode(Window window, boolean isLight) {
//        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
//            Log.e("StatusBarTextColorUtils", "系统级别太低，不设置状态栏颜色");
//            return;
//        }
        ...
        if (brand == FitUtils.Brand.XIAO_MI) {
            // 小米 状态栏颜色
            lightModeSetSuccess = setStatusBarLightModeMIUI(window, isLight);
        } else if (brand == FitUtils.Brand.MEI_ZU) {
            // 魅族 状态栏颜色
            lightModeSetSuccess = setStatusBarLightModeFlyMe(window, isLight);
        } else {
            // Android 6.0 之后的原生方式
            setStatusBarLightModeOrigin(window, isLight);
        }
...
//    }
    ...
    private static void setStatusBarLightModeOrigin(Window window, boolean lightMode) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            Log.e("StatusBarTextColorUtils", "系统级别太低，不设置状态栏颜色,直接全屏");
